...
tf.app.flags.DEFINE_string('gpu_list', '1', 'input number of GPUs')
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            boxes, timer = detect(score_map=score,
                                  ...
                                  timer=timer)
            boxes = []
            box1 = [136 * ratio_w, 76 * ratio_h, 170 * ratio_w, 76 * ratio_h,
                    170 * ratio_w, 93 * ratio_h, 136 * ratio_w, 93 * ratio_h, 0.5]
            box2 = [120 * ratio_w, 96 * ratio_h, 185 * ratio_w, 96 * ratio_h,
                    185 * ratio_w, 118 * ratio_h, 120 * ratio_w, 118 * ratio_h, 0.5]
            boxes.append(box1)
            boxes.append(box2)
...
if __name__ == '__main__':
    frame = cv2.imread('test_imgs/fold.jpg')
    pos = [136, 76]
    size = [39, 18]
    recognize(frame, pos, size)
